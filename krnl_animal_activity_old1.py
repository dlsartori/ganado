...
class AnimalActivity(Activity):     # Abstract Class (no lleva ningun instance attributte). NO se instancia.
    ...
    def __call__(self, caller_obj=None):
        ...
        #       f'kwargs: {kwargs}')
        self.outerObject = caller_obj  # caller_obj es instance de Animal, Tag, etc. NO PUEDE SER class.
        return self


    # def __call__00(self, caller_obj=None):
    #     """            *** This method is inherited and can be accessed from derived classes. Cool!! ****
    #     __call__ is invoked when an instance is followed by parenthesis (instance_obj()) or when a property is defined
    #     on the instance, and the property is invoked (inventory.get(): in this case __call__ is invoked for the object
    #     associated with inventory property before making the get() call. Then, outerObject is properly initialized.)
    ...
    #     """
    #
    #     # TODO(cmt): 10-Jan-24. Implementing __call__() with inner function.
    #     caller_object = caller_obj      # This value used within func as a nonlocal variable. NOT WORKING!!
    #
    ...
    temp = getRecords('tblAnimalesActividadesNombres', '', '', None, 'fldID', 'fldName', 'fldFlag',
                      ...
                      'fldFilterField', 'fldAdHocMethods')
    _activitiesDict = {}           # {Nombre Actividad: ID_Actividad, }
    _isInventoryActivity = {}      # {Nombre Actividad: isInventory (True/False), }
    ...
    def _setCategory(self, *args, excluded_fields=None, execute_fields=None, **kwargs):
        ...
        if not outerObj or type(outerObj) is type:          # Only operates with objects. No good with classes.
            raise TypeError(f'_setCategory() cannot be called by a class ({outerObj}). Only to be called by an Activity object.')
        ...
        if categoryID not in outerObj.getCategories().values():  # Class access to attr. to generalize to obj or class.
            ...
            # next((outerObj.categories[j] for j in outerObj.categories if removeAccents(str(j)) == categID), None)
        ...
        # No prob with this call on category.get(), TODO: as long as set_category=False to avoid recursion.
        lastCategory = outerObj.category.get(mode='memory', set_category=False)
        # local_dict = self.get_local_uids_dict()[type(self.outerObject)]
        # lastCategory = local_dict[self.outerObject.ID].record.get('fldFK_Categoria')
        # krnl_logger.info(f'--------- category.set() -> last_category for {self.outerObject}: {lastCategory}')
        ...
        if categoryID and (categoryID in self.permittedDict()[lastCategory] or enforce):
            ...
            if isinstance(retValue, int):
                ...
                outerObj._fldFlagCastrado = eventDate.timestamp()  # Converts to int to store flagCastrado.
                ...
                if self.supports_mem_data():
                    ...
                    if not memVal or memVal.get('fldDate', 0) < eventDate:
                        ...
                        mem_object = Activity.ActivitiesMemoryData(**self._mem_data_params, values=values)
                        # set_mem_data uses a Lock() to prevent concurrent write attempts by different threads
                        self.set_mem_data(mem_object)
    ...
    def _getCategory(self, sDate='', eDate='', *args, mode='mem', id_type='id', full_record=False, uid=None,
                     set_category=False, **kwargs):
        """
        ...
        if 'mem' in modeArg:
            ...
            if isinstance(tmp, dict):
                val = tmp.get(fldName)
                computedCategory = self._compute(initial_category=val, uid=uid)
                if computedCategory == val:
                    if val in self.category_names():
                        if not full_record:
                            return self.category_names()[val] if 'name' in id_type else val
                        return tmp      # Returns full data dictionary.
                    else:
                        print(f'############### getCategory category is not valid!!: {val}.   ')
                    return tmp

        # self.outerObject will be an instance of Bovine, Caprine, etc., or the CLASS Bovine, Caprine, etc.
        ...
        qryTable = self._getRecordsLinkTables(tblRA, tblLink, tblData, outer_obj_id=uid)
        if isinstance(qryTable, DataTable):
            if qryTable.dataLen == 1:
                ...
                if not isinstance(result, DataTable):
                    krnl_logger.error(f'ERR_SYS: Error reading data from DataTable {qryTable.tblName}. e = {result}.')
                    return None
            if result.dataLen == 1:
                categID = result.getVal(0, fldName)
                # Checks against existing category and re-compute if they are different.
                computedCategory = self._compute(initial_category=categID, uid=uid)   # Computes without calling set().
            else:
                return result       # Returns DataTable with all records found. Not used for now (May-24)...
            ...
            if categID != computedCategory:
                if set_category:
                    ...
                    tblLink.setVal(0, fldID=None, fldFK=getattr(outer_object, 'ID', uid))
                    tblData.setVal(0, fldID=None, fldFK_Categoria=computedCategory,
                                   fldComment='Category set from _getCategory().')
                    tblRA.setVal(0, fldFK_NombreActividad=self._activityID)
                    idActividadRA = self._createActivityRA(tblRA, tblLink, tblData, uid=uid, **kwargs)  # Sets new category.
                    if isinstance(idActividadRA, int):
                        ...
                        if isinstance(mem_rec, dict):
                            mem_rec[fldName] = computedCategory          # Updates record fields.
                            mem_rec['fldFK_Actividad'] = idActividadRA
                            mem_rec['fldDate'] = tblData.getVal(0, 'fldDate')
                            values = list(mem_rec.values())
                            self.set_mem_data(Activity.ActivitiesMemoryData(**self._mem_data_params, values=values),
                                              uid=uid)
                    else:
                        krnl_logger.error(f'ERR_DBAccess: could not update Animal Category in database. Computed '
                                          f'category not saved.')
            if full_record:
                result.setVal(0, fldFK_Categoria=computedCategory)
                retValue = result.unpackItem(0)         # Returns full dictionary of tblData fields.
...
class InventoryActivityAnimal(AnimalActivity):
    ...
    @classmethod
    def dict_updating(cls):
        return cls.__dict_updating

    @classmethod
    def dict_update_starts(cls):
        cls.__dict_updating = True

    @classmethod
    def dict_update_ends(cls):
        cls.__dict_updating = False
    ...
    def set(self, *args: DataTable, execute_fields=None, excluded_fields=(), **kwargs):
        ...
        if isinstance(retValue, int) and self._supportsPA:
            ...
            # print(f'({lineNum()}) returning from _paMatchAndClose() in Inventory set(). '
            #       f'outerAttr still at {self._outerAttr()}')
    ...
    def get(self, sDate='', eDate='', *args, **kwargs):
        ...
        @return: Object DataTable with information from queried table or statusID (int) if mode=val
        """
        # print(f'@@@@@@@@@@@@@ Inventory.get() outerObj: {self.outerObject}')
    ...
    @classmethod
    def _memory_data_init_last_rec(cls, obj_class, active_uids: set, **kwargs):
        ...
        @return: None
        """
        data_fld_name = 'fldDaysToTimeout'
        ...
        if local_uids_dict is not None:    # This is None for classes that don't support_mem_data(). If so, exits.
            if obj_class not in local_uids_dict:
                local_uids_dict[obj_class] = {}  # Creates entry (1 uids_dict) for obj_class within local_uids_dict.

            # Check if any cls has _animalClassID attribute defined (Bovine, Caprine, etc. will have it).
            # Loads 1 dictionary at a time for: classes with _animalClassID == cls._animalClassID or for classes with
            ...
            if not hasattr(cls, '_animalClassID') or cls.animalClassID() == obj_class.animalClassID():
                ...
                fields = [f for f in cls._mem_data_params['field_names'] if f in tblData_fields]
                loaded_dict = cls._load_memory_data_last_rec(tbl=cls.tblDataName(), keys=active_uids, flds=fields)

                # Now, get fldDaysToTimeout for active_uids.
                uid_db_fld_name = getFldName(tbl, uid_fld_name)
                sql = f'SELECT "{uid_db_fld_name}", "{getFldName(tbl, data_fld_name)}" ' \
                      f'FROM "{getTblName(tbl)}" WHERE "{uid_db_fld_name}" IN {str(tuple(active_uids))}; '
                temp = dbRead(tbl, sql)
                if not isinstance(temp, DataTable):
                    return None
                tblObj_cols = temp.getCols(uid_fld_name, data_fld_name)
                tblObj_dict = dict(zip(tblObj_cols[0], tblObj_cols[1]))  # {uid: fldDaysToTimeout, }
                del temp

                # Updates Activity class __local_active_uids_dict with memory data assembled from db reads above.
                temp_dict = {}
                for uid in loaded_dict:
                    uid_list = list(loaded_dict[uid])
                    uid_list.append(tblObj_dict[uid])  # Appends DOB to values list.
                    temp_dict[uid] = cls.ActivitiesMemoryData(**cls._mem_data_params, values=uid_list)

                # Now resolves potential duplicate keys in the dicts, picking (k, v) with that latest fldDate in v.
                obj_class_dict = local_uids_dict[obj_class]
                if obj_class_dict and any(k in obj_class_dict for k in temp_dict):
                    # update() the case of set_mem_data() inserting data in the dict. BEFORE this function ends.
                    repeat_dict = {k: v for (k, v) in temp_dict.items() if k in obj_class_dict}
                    if repeat_dict:
                        last_values = {k: max((repeat_dict[k], obj_class_dict[k]),
                                              key=lambda x: x.record.get('fldDate', 0)) for k in repeat_dict}
                        temp_dict.update(last_values)

                # update() because __local_active_uids_dict may be modified by another thread while this is running.
                obj_class_dict.update(temp_dict)  # TODO(cmt): update() it's atomic, they say...
                dicto = {"uid": list(temp_dict.items())[0][1].record}
                print(f'MEMORY DATA for {cls.__name__}: {len(temp_dict)} uid items. Item form: '
...
class CastrationActivityAnimal(AnimalActivity):
    ...
    def set(self, *args, event_date=0):
        ...
        @param event_date: datetime (int). 0, False, None or '' -> Castration = NO
        ...
        if event_date:
            if not isinstance(event_date, (int, float)):
                event_date = event_date.timestamp() if isinstance(event_date, datetime) else 1
        else:
            event_date = 0

        outer_obj = self.outerObject
        # with self.__lock:
        outer_obj.isCastrated = event_date        # event_date es variable local, para aceptar re-entry.
        tblObjects = DataTable(self.tblObjName())
        tblObjects.setVal(0, fldFlagCastrado=outer_obj.isCastrated, fldFK_UserID=sessionActiveUser,
                          fldID=outer_obj.recordID)
        ...
        tblData = DataTable(self.__tblDataName, *args, fldDate=outer_obj.isCastrated)
...
class ParturitionActivityAnimal(AnimalActivity):
    ...
    def set(self, *args, event_date=None, parturition_status=1, execute_fields=None, excluded_fields=()):
        ...
        tblRA = DataTable(self._tblRAName, *args)
        tblLink = DataTable(self._tblLinkName, *args)
        tblData = DataTable(self.__tblDataName, fldDate=event_date, fldFK_ResultadoDeParicion=parturition_status)
    ...
    def get(self):
        ...
        record keeps the full data of each parturition for the Animal.
         ...
         """
        return self._getRecordsLinkTables(self._tblRAName, self._tblLinkName, self._tblDataName,
                                          activity_list=(self._activityID, ))
        # return tbl  # tbl.getCol('fldID')    # Returns list of RA Activities that are Parturition, for Animal.
...
class TagActivityAnimal(AnimalActivity):
    ...
    # @staticmethod
    # def __tagRegister():                # Tag.getRegisterDict() IS DEPRECATED. DO NOT USE!
    ...
    def initializeTags(self, *args, **kwargs):              # TODO: COMPLETE FIXING THIS ONE.
        """
        ...
        tagIDs = self.outerObject.getIdentifiers()      # list of tag UIDs stored in tblAnimales, tblDispositivos, etc.
        objList = []
        for i in tagIDs:
            tag_class = self.outerObject._myTagClass()
            obj = tag_class.getObject(i)
            if isinstance(obj, tag_class):
    ...
    def assignTags(self, *args: DataTable, comm_type='Comision', tag_list=None, **kwargs):
        """
        Assigns Tags in *args to Animal. Updates tables in DB.
        @param tag_list: iterable with Tag objects to assign to Animal (list or tuple)
        ...
        if tags:
            ...
            sql = f"SELECT * FROM {self.__tblObjectDBName} WHERE {getFldName('fldObjectUID')} IN " \
                  f"{str(tuple([t.ID for t in tags]))}"
            tagRecords = dbRead(self.__tblObjectsName, sql)
            if isinstance(tagRecords, str):
                retValue = f'ERR_DBAccess: Cannot read table {self.__tblObjectsName}'
                krnl_logger.error(retValue, stack_info=True, exc_info=True)
                return retValue

            uid_col = tagRecords.getCol('fldObjectUID')
            wrtTables = []
            permittedAssignStatus = (1, 3, None, '')  # Alta:1, Decomisionada:3, None: sin status.
            ...
            commentLink = __tblLink.getVal(0, 'fldComment', '') or "" + f' / Activity: {activityName}; {timeStamp}'
            ...
            for t in tags:
                if isinstance(t, Tag) and t.status.get() in permittedAssignStatus:
                    if t.assignedToClass != self.outerObject.__class__.__name__:
                        ...
                        if t.ID in uid_col:
                            ...
                            retValue = f'ERR_Sys: Tag Number {t.tagNumber} / ID={t.ID} not found.'
                            krnl_logger.error(retValue, stack_info=True)
                            continue   # Continua a procesar el siguiente tag.
    ...
    # @classmethod
    # def isTagAssigned00(cls, tag):  # cls is Bovine, Caprine, etc.
    #     """
    #     Informs whether tag is assigned to an object of type cls (by returning the object tag is tied to).
    #     If tag is not found in the first pass, reloads ALL tags for all objects from db to account for potential
    #     database updates due to replication.
