...
""" 10Jul24: This is the last version of custom_types with full implementation of DataTable class.
Going forward, the transition from DataTable to pd.DataFrame start with the gradual deprecation of DataTables.
"""
...
DATATABLE_USE_RECORDCLASS = parse_cmd_line('use_recordclass') if parse_cmd_line('use_recordclass') is not None else True
...
def df_getrecords(tbl_name, *args, db_name=None, sql=None, where_str='', index_col=None, coerce_float=True,
    ...
    if not args:
        ...
        flds = {k: v for k, v in fld_names.items() if (k in args or v in args)} # Filters only valid field names for tbl
        fields_list = list(flds.values())
        fields_list = tuple([f'"{j}"' for j in fields_list])

    sql = f'SELECT {fields_list} FROM "{getTblName(tbl_name)}" ' + (where_str if isinstance(where_str, str) else '') + ';'
    return pd.read_sql_query(sql, con, index_col=index_col, coerce_float=coerce_float, params=params,
                                  parse_dates=parse_dates, chunksize=chunksize, dtype=dtype)
...
def getColors():
    ...
    if isinstance(temp, DataTable):
        color_dict = {r[1]: (r[0], r[2]) for r in temp.dataList}
        return color_dict
    return {}
