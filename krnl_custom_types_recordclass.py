...
class DataTable(object):
    ...
    def __init__(self, tblName=None, dList=None, keyFieldNames=None, *args, db_name='', **kwargs):
        ...
        if isinstance(tblInfo, str):
            ...
            DataRecord = recordclass("__DataRecord", self._fldNames)  # KeyFieldNames is the class.
            if self._dataList:
                ...
                if self.__use_recordclass:      # DataTable using recordclass
                    ...
                    self._dataList = [j for j in self._dataList if isinstance(j, (tuple, list, DataRecord))]
                    # Converts items to __DataRecord ONLY IF they are tuple, list and NOT if they are already __DataRecord.
                    self._dataList = [(DataRecord(*j) if not isinstance(j, DataRecord) else j) for j in self._dataList]
    ...
    def setVal(self, recIndex=0, fName=None, val=None, **kwargs):
        ...
        if self.__isValidFlag and isinstance(recIndex, int):
            if kwargs:
                ...
                #             try:
                #                 setattr(self._dataList[recIndex], k, v)  # _dataList[recIndex] is a __DataRecord insntance
    ...
    def getVal(self, recIndex=0, fName=None, defaultVal=None, *, fldID=None):
        ...
        #         return getattr(self._dataList[recIndex], fName)   # Each _dataList[recIndex] is a __DataRecord insntance.
        #     except (AttributeError, IndexError, TypeError):
        #         return defaultVal
